#!/bin/bash
# [NGC DOWNLOAD job run --config "fine_tune_rn_r_x" "python -m experiments.train_path \"rn(r(x))\"";
...
NGC job run --config "fine_tune_rn_Er_a_x" "python -m experiments.train_path \"rn(Er(a(x)))\"";
[cloud5] job run --config "fine_tune_F_RC_x" "python -m experiments.train_path \"F(RC(x))\"";
