...
def sample(min_val=0, center=0.5, max_val=1, geometric=False, plot_range=None, generator=None):
    ...
    def wrapper(transform):
        class RandomSampler:
            ...
            def __init__(self, transform, plot_range, generator, geometric):
            ...
            def sample_with_intensity(self, x, intensity=0.0, **kwargs):
                span = max(max_val - center, center - min_val)
                options = []
                if center + intensity*span <= max_val: options.append(center + intensity*span)
                if center - intensity*span >= min_val: options.append(center - intensity*span)
                return self.transform(x, random.choice(options), **kwargs)

            def __str__(self):
                return self.__name__

        return RandomSampler(transform, plot_range, generator, geometric)
...
@sample(0.9, 1.0, 1.1, plot_range=(0.8, 1.2))
...
@sample(0.0, 0.0, 0.1, geometric=True)
...
@sample(0.0, 0.0, 1.0, geometric=True, plot_range=(0, 1))
def flip(x, val=0.0):
    if val < 0.1:
        return x
...
@sample(0.0, 0.0, 0.2, plot_range=(0.01, 0.3))
...
@sample(0.5, 1, 1, geometric=True, plot_range=(0.2, 1))
def crop(x, p=0.6):
    N, C, H, W = x.shape
    H_c, W_c = int((H * W * p) ** 0.5), int((H * W * p) ** 0.5)
    x_coord = int(random.uniform(0, H - H_c))
...
## NOT DIFFERENTIABLE ##
@sample(50, 100, 100, plot_range=(10, 100))
