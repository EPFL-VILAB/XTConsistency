...
def main():
    ...
    def mixed_loss(pred, target):
        ...
        edge = mse_loss(normal2edge(pred, checkpoint=False), target)
        # curvature = torch.tensor(0.0, device=pred.device) if curvature_weight == 0.0 else \
        #                 mse_loss(curve_cycle(pred), curvature_model(pred))
        depth = mse_loss(depth_cycle(pred, checkpoint=False), depth_model(pred, checkpoint=False))

        if pred.requires_grad:
            edge_weight, depth_weight = calculate_weight(model, edge, depth)
        else:
            edge_weight, depth_weight = [torch.tensor(0.0, device=pred.device)]*2
        return edge_weight*edge + depth_weight*depth, (edge.detach(), edge_weight.detach(), depth.detach())
    ...
    logger.add_hook(partial(jointplot, logger=logger, loss_type="edge_coeff"), feature="val_edge_coeff", freq=1)
    ...
    plot_images(
        model, logger, test_set, ood_images, mask_val=-1.0, 
        loss_models={
            # "curvature": curvature_model, 
            "depth": depth_model, 
            # "curvature_cycle": curve_cycle, 
