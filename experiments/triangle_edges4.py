...
def main(curvature_step=0, depth_step=0):
    ...
    def mixed_loss(pred, target):
        ...
        R, r = reshade2normal, normal2reshade
        G, g = depth2normal, depth_model

        ax, y, y_hat = target, pred, target
        mse_loss = lambda x, y: ((x*mask.float() -y*mask.float())**2).mean()

        cycle_curve = mse_loss(F(f(y)), y)
        cycle_depth = mse_loss(G(g(y)), y)
        cycle_reshade = mse_loss(R(r(y)), y)
        ...
        return cycle_curve + cycle_depth + cycle_reshade + edge, (cycle_curve.detach(), cycle_depth.detach(), cycle_reshade.detach(), edge.detach())

    # LOGGING
    logger = VisdomLogger("train", env=JOB)
    logger.add_hook(lambda x: logger.step(), feature="loss", freq=10)
    logger.add_hook(partial(jointplot, logger=logger, loss_type="cycle_curve_loss"), feature="val_cycle_curve_loss", freq=1)
    logger.add_hook(partial(jointplot, logger=logger, loss_type="cycle_depth_loss"), feature="val_cycle_depth_loss", freq=1)
    logger.add_hook(partial(jointplot, logger=logger, loss_type="cycle_reshade_loss"), feature="val_cycle_reshade_loss", freq=1)
    ...
    plot_images(model, logger, test_set, ood_images, mask_val=-1.0,
        loss_models={
            ...
            "g(y) depth": depth_model,
            "r(y) reshade": normal2reshade,
            "NI(y) edge": normal2edge,
            "F(f(y)) curve cycle": lambda x: curvature2normal(curvature_model(x)),
            "F(f(y)) depth cycle": lambda x: depth2normal(depth_model(x)),
            "F(f(y)) reshade cycle": lambda x: reshade2normal(normal2reshade(x)),
        },
        loss_targets=False
    )
    ...
    for epochs in range(0, 800):
        ...
        (cycle_curve_data, cycle_depth_data, cycle_reshade_data, edge_data) = model.fit_with_metrics(
            train_set, loss_fn=mixed_loss, logger=logger
        )
        logger.update("train_cycle_curve_loss", np.mean(cycle_curve_data))
        logger.update("train_cycle_depth_loss", np.mean(cycle_depth_data))
        logger.update("train_cycle_reshade_loss", np.mean(cycle_reshade_data))
        logger.update("train_edge_loss", np.mean(edge_data))

        val_set = itertools.islice(val_loader, val_step)
        (cycle_curve_data, cycle_depth_data, cycle_reshade_data, edge_data) = model.predict_with_metrics(
            val_set, loss_fn=mixed_loss, logger=logger
        )
        logger.update("val_cycle_curve_loss", np.mean(cycle_curve_data))
        logger.update("val_cycle_depth_loss", np.mean(cycle_depth_data))
        logger.update("val_cycle_reshade_loss", np.mean(cycle_reshade_data))
