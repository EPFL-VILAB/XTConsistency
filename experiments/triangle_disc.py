...
def main(disc_step=0.01):
    ...
    def mixed_loss(pred, target, data):
        ...
        disc_loss, _ = disc.loss(disc.forward(pred), torch.tensor(0, device=pred.device).expand(pred.shape[0]))
        ...
        return mse + disc_weight*disc_loss, (mse.detach(), disc_loss.detach(),)
    ...
    for epochs in range(0, 800):
        ...
        (disc_data, curvature_data, depth_data) = model.fit_with_metrics(
        ...
        (disc_data, curvature_data, depth_data) = model.predict_with_metrics(
