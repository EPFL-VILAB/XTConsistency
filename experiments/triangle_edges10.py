...
def main(curvature_step=0, depth_step=0):
    ...
    def mixed_loss(pred, target):
        ...
        inversecurve_normal = mse_loss(EC(ax), f(y))        
        ...
        return inversecurve_normal, (inversecurve_normal.detach(),)

    # LOGGING
    logger = VisdomLogger("train", env=JOB)
    logger.add_hook(lambda x: logger.step(), feature="loss", freq=10)
    logger.add_hook(partial(jointplot, logger=logger, loss_type="inversecurve_normal_loss"), feature="val_inversecurve_normal_loss", freq=1)
    ...
    for epochs in range(0, 800):
        ...
        (inversecurve_normal_data,) = model.fit_with_metrics(
        ...
        logger.update("train_inversecurve_normal_loss", np.mean(inversecurve_normal_data))

        val_set = itertools.islice(val_loader, val_step)
        (inversecurve_normal_data,) = model.predict_with_metrics(
        ...
        logger.update("val_inversecurve_normal_loss", np.mean(inversecurve_normal_data))
