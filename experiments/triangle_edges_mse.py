...
def main(curvature_step=0, depth_step=0):
    ...
    def mixed_loss(pred, target, data):
        ...
        mse = torch.tensor(0.0, device=pred.device) if mse_weight == 0.0 else \
        ...
        def dest_transforms(x):
            ...
            edge = torch.FloatTensor(edge).unsqueeze(0)
            return edge
        edges = torch.stack([image for image in data], dim=0)

        edge = mse_loss(normal2edge(pred), edges.to(pred.device))

        return edge + mse_weight*mse + curvature_weight*curvature + depth_weight*depth, (mse.detach(), curvature.detach(), depth.detach(), edge.detach())
    ...
    for epochs in range(0, 800):
        ...
        (mse_data, curvature_data, depth_data, edge_data) = model.fit_with_metrics(
        ...
        (mse_data, curvature_data, depth_data, edge_data) = model.predict_with_metrics(
