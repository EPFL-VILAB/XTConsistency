...
def main(path_str, src_task='rgb'):
    ...
    for i, transfer in enumerate(path):
        ...
        logger.add_hook(partial(jointplot, logger=logger, loss_type=f"mse_loss_{prefix}"), feature=f"val_mse_loss_{prefix}", freq=1)
        logger.add_hook(partial(jointplot, logger=logger, loss_type=f"percep_loss_{prefix}"), feature=f"val_percep_loss_{prefix}", freq=1)
        ...
        for epochs in range(0, 10):
            ...
            (train_mse_data, train_percep_data) = model.fit_with_metrics(train_set, logger=logger)
            logger.update(f"train_mse_loss_{prefix}", np.mean(train_mse_data))
            logger.update(f"train_percep_loss_{prefix}", np.mean(train_percep_data))

            val_set = itertools.islice(val_loader, val_step)
            (val_mse_data, val_percep_data) = model.predict_with_metrics(val_set, logger=logger)
            logger.update(f"val_mse_loss_{prefix}", np.mean(val_mse_data))
            logger.update(f"val_percep_loss_{prefix}", np.mean(val_percep_data))
            if epochs == 0:
            ...
            model.model.save(get_finetuned_model_path(path[:i+1]))
