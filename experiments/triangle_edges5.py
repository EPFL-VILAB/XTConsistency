...
def main(curvature_step=0, depth_step=0):
    ...
    def mixed_loss(pred, target):
        ...
        return cycle + curve_edge + norm_edge, (cycle.detach(), curve_edge.detach(), norm_edge.detach())
    ...
    for epochs in range(0, 800):
        ...
        (cycle_data, curve_edge_data, norm_edge_data) = model.fit_with_metrics(
        ...
        logger.update("train_norm_edge_loss", np.mean(norm_edge_data))

        val_set = itertools.islice(val_loader, val_step)
        (cycle_data, curve_edge_data, norm_edge_data) = model.predict_with_metrics(
        ...
        logger.update("val_norm_edge_loss", np.mean(norm_edge_data))
