...
def run_eval_suite(name, dest_task=tasks.normal, graph_file=None, model_file="unet_percepstep_0.1.pth", logger=None, sample=800, show_images=False, old=False):
    ...
    logger.text (name + ": " + str(dataset.evaluate(model, sample=sample, logger=logger, show_images=show_images)))
...
if __name__ == "__main__":
    ...
    run_eval_suite("unet-b-100k", model_file=f"{SHARED_DIR}/results_SAMPLEFF_baseline100k_8/n.pth", logger=logger)
    run_eval_suite("unet-pc-100k", graph_file=f"{SHARED_DIR}/results_SAMPLEFF_consistency100k_4/graph.pth", logger=logger)
